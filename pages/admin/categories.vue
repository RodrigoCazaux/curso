  ...
  <div class="space-y-8">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
      <div>
        ...
        <p class="text-sm text-gray-500">
          Crea, edita y elimina las categorías que organizan tu catálogo de productos.
        </p>
      </div>
      <button
        type="button"
    ...
    <div class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
      ...
      <form class="grid gap-4 md:grid-cols-2" @submit.prevent="handleSubmit">
        <div class="md:col-span-1">
          <label class="block text-sm font-medium text-gray-700 mb-1" for="category-name">Nombre</label>
          <input
            id="category-name"
            v-model.trim="form.name"
            type="text"
            required
            placeholder="Ej. Tintos"
        ...
        <div class="md:col-span-1">
          <label class="block text-sm font-medium text-gray-700 mb-1" for="category-description">Descripción</label>
          <input
    ...
    <div class="rounded-lg border border-gray-200 bg-white shadow-sm">
      <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
        <div>
          <h2 class="text-base font-semibold text-gray-900">Listado de categorías</h2>
          <p class="text-xs uppercase text-gray-500">{{ categoriesList.length }} categorías registradas</p>
        </div>
      </div>
      ...
      <table v-else class="min-w-full divide-y divide-gray-100">
        ...
        <tbody class="divide-y divide-gray-100 bg-white">
          <tr v-for="category in categoriesList" :key="category.id" class="hover:bg-gray-50 transition">
            ...
            <td class="px-6 py-4 text-sm text-right space-x-3">
              ...
              <button
                type="button"
                class="text-rose-600 hover:text-rose-500 disabled:opacity-50 disabled:cursor-not-allowed"
      ...
      <div v-if="!isLoading && !categoriesList.length" class="px-6 py-10 text-center text-sm text-gray-500">
        Aún no tienes categorías registradas. Usa el formulario superior para crear la primera.
      </div>
    </div>
  ...
  methods: {
    ...
    async loadCategories() {
      try {
        ...
        this.showFeedback("error", "No se pudieron cargar las categorías. Intenta nuevamente.");
      } finally {
    ...
    async handleSubmit() {
      ...
      try {
        ...
        if (this.isEditing) {
          ...
          this.showFeedback("success", "Categoría actualizada correctamente.");
        } else {
          await this.addCategory(payload);
          this.showFeedback("success", "Categoría creada correctamente.");
        }
        this.resetForm();
      } catch (error) {
        this.showFeedback(
          "error",
          error?.message || "Ocurrió un error al guardar la categoría. Intenta nuevamente."
        );
      } finally {
    ...
    async confirmDelete() {
      if (!this.categoryToDelete?.id || this.isDeleting) return;
      try {
        this.isDeleting = true;
        await this.deleteCategory(this.categoryToDelete.id);
        this.showFeedback("success", "Categoría eliminada correctamente.");
        this.closeDeleteModal();
      } catch (error) {
        console.error("Error al eliminar categoría:", error);
        this.showFeedback(
          "error",
          error?.message || "No se pudo eliminar la categoría. Intenta nuevamente."
        );
      } finally {
